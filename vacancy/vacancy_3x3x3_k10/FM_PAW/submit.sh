#!/bin/bash
#SBATCH --job-name=fe_vacancy_fm
#SBATCH --nodes=1
#SBATCH --ntasks=128
#SBATCH --ntasks-per-node=128
#SBATCH --cpus-per-task=1 # For OpenMP, but we force OMP_NUM_THREADS=1
#SBATCH --time=025:00:00  # Increased time for relaxation
#SBATCH --partition=regular
#SBATCH --output=fe_vacancy_fm_3x3x3_%j.slurm.out
#SBATCH --error=fe_vacancy_fm_3x3x3_%j.slurm.err
#SBATCH --mem=500G
...
echo "Running FERROMAGNETIC vacancy calculation"
...
echo "Usage: sbatch submit_vacancy_fm.sh vacancy_fm_input.in vacancy_fm_output.out"
exit 1
fi
...
echo "Running QE FM vacancy calculation with input=$QE_INPUT_FILE, output=$QE_OUTPUT_FILE"

#!/bin/bash
#SBATCH --job-name=fe_bulk_nm_paw_scf
#SBATCH --nodes=1
#SBATCH --ntasks=128
...
#SBATCH --output=fe_bulk_nm_paw_scf_%j.slurm.out
#SBATCH --error=fe_bulk_nm_paw_scf_%j.slurm.err
#SBATCH --mem=150G
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_scf.sh fe_nm_bulk_3x3x3_k101010_paw_scf.in fe_nm_bulk_3x3x3_k101010_paw_scf.out"
    exit 1
fi

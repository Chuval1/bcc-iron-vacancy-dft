#!/bin/bash
#SBATCH --job-name=fe_bulk_nm_uspp_scf
#SBATCH --nodes=1
#SBATCH --ntasks=128
#SBATCH --ntasks-per-node=128
#SBATCH --cpus-per-task=1 # For OpenMP, but we force OMP_NUM_THREADS=1
#SBATCH --time=06:00:00 # SCF should be much faster
#SBATCH --partition=regular
#SBATCH --output=fe_bulk_nm_uspp_scf_%j.slurm.out
#SBATCH --error=fe_bulk_nm_uspp_scf_%j.slurm.err
#SBATCH --mem=150G
...
echo "Running NM bulk SCF calculation"
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_scf.sh fe_nm_bulk_3x3x3_k101010_uspp_scf.in fe_nm_bulk_3x3x3_k101010_uspp_scf.out"
    exit 1
fi
...
echo "Running QE NM bulk SCF with input=$QE_INPUT_FILE, output=$QE_OUTPUT_FILE"

#!/bin/bash
#SBATCH --job-name=fe_bulk_fm_uspp_relax
#SBATCH --nodes=1
#SBATCH --ntasks=128
...
#SBATCH --output=fe_bulk_fm_uspp_relax_%j.slurm.out
#SBATCH --error=fe_bulk_fm_uspp_relax_%j.slurm.err
#SBATCH --mem=150G
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_relax.sh fe_fm_bulk_3x3x3_k101010_uspp_relax.in fe_fm_bulk_3x3x3_k101010_uspp_relax.out"
    exit 1
fi

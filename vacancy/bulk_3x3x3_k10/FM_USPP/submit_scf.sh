#!/bin/bash
#SBATCH --job-name=fe_bulk_fm_uspp_scf
#SBATCH --nodes=1
#SBATCH --ntasks=128
...
#SBATCH --output=fe_bulk_fm_uspp_scf_%j.slurm.out
#SBATCH --error=fe_bulk_fm_uspp_scf_%j.slurm.err
#SBATCH --mem=150G
...
echo "Running FM bulk SCF calculation"
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_scf.sh fe_fm_bulk_3x3x3_k101010_uspp_scf.in fe_fm_bulk_3x3x3_k101010_uspp_scf.out"
    exit 1
fi
...
echo "Running QE FM bulk SCF with input=$QE_INPUT_FILE, output=$QE_OUTPUT_FILE"

#!/bin/bash
#SBATCH --job-name=fe_bulk_fm_paw_scf
#SBATCH --nodes=1
#SBATCH --ntasks=128
#SBATCH --ntasks-per-node=128
#SBATCH --cpus-per-task=1 # For OpenMP, but we force OMP_NUM_THREADS=1
#SBATCH --time=03:30:00 # SCF should be much faster
#SBATCH --partition=regular
#SBATCH --output=fe_bulk_fm_paw_scf_%j.slurm.out
#SBATCH --error=fe_bulk_fm_paw_scf_%j.slurm.err
#SBATCH --mem=150G
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_scf.sh fe_fm_bulk_3x3x3_k101010_paw_scf.in fe_fm_bulk_3x3x3_k101010_paw_scf.out"
    exit 1
fi

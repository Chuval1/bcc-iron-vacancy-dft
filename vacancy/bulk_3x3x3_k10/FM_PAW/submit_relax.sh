#!/bin/bash
#SBATCH --job-name=fe_bulk_fm_paw_relax
#SBATCH --nodes=1
#SBATCH --ntasks=128
#SBATCH --ntasks-per-node=128
#SBATCH --cpus-per-task=1 # For OpenMP, but we force OMP_NUM_THREADS=1
#SBATCH --time=05:00:00 # Bulk should be faster than vacancy
#SBATCH --partition=regular
#SBATCH --output=fe_bulk_fm_paw_relax_%j.slurm.out
#SBATCH --error=fe_bulk_fm_paw_relax_%j.slurm.err
#SBATCH --mem=150G
...
echo "Running FM bulk relaxation calculation"
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "Usage: sbatch submit_relax.sh fe_fm_bulk_3x3x3_k101010_paw_relax.in fe_fm_bulk_3x3x3_k101010_paw_relax.out"
    exit 1
fi
...
echo "Running QE FM bulk relaxation with input=$QE_INPUT_FILE, output=$QE_OUTPUT_FILE"
